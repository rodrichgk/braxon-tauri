...
async function main() {
  ...
  try {
    const col = await conn.query(
      `SELECT COLUMN_NAME, DATA_TYPE FROM _USER_COLUMNS WHERE TABLE_NAME = 'LigCde' AND COLUMN_NAME = 'SuiviGar_AncNoInterv'`
    );
    console.log('Column info:', col);

    // Try a few WHERE variants.
    for (const clause of [
      `WHERE SuiviGar_AncNoInterv <> 0`,
      `WHERE NOT (SuiviGar_AncNoInterv = 0)`,
      `WHERE SuiviGar_AncNoInterv IS NOT NULL AND SuiviGar_AncNoInterv <> 0`,
    ]) {
      try {
