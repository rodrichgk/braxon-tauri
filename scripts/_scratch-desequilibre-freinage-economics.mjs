...
async function main() {
  ...
  try {
    const rows = await conn.query(
      `SELECT l.NoInt_Ligcde, l.CodeArt, l.PrixHT, l."Soldée" FROM LigCde l LEFT JOIN ArticleMeteor am ON l.CodeArt = am.CodeArt
       WHERE am.Famille = 'MK100' AND l.Observations LIKE '%DESEQUILIBRE%FREINAGE%' LIMIT 3000`
    );
    console.log(`Total MK100 "desequilibre de freinage" jobs: ${rows.length}`);
    ...
    console.log(`Repaired: ${repaired}, other: ${other}`);
