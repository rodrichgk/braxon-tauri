...
async function main() {
  ...
  try {
    const lig = await conn.query(`SELECT NoInt_Ligcde, NoInt_cde FROM LigCde WHERE NoIntervention = '17475301'`);
    if (!lig.length) { console.log('Job 17475301 not found'); return; }
    const ligcdeId = lig[0].NOINT_LIGCDE;
    console.log('Job 17475301 -> ligcde', ligcdeId);
    ...
    const outPath = 'scripts/_scratch-snapshot-17475301-before.json';
