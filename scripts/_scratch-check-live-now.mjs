...
async function main() {
  ...
  try {
    const top = await conn.query('SELECT NoInt_interv, NoIntLigcde, NoIntTechn, TypeCode, "Date", HeureInterv FROM Intervention ORDER BY NoInt_interv DESC LIMIT 5');
    console.log('=== Top 5 ids right now ===');
