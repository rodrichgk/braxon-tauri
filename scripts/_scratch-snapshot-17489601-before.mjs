...
async function main() {
  ...
  try {
    const lig = await conn.query(`SELECT NoInt_Ligcde, NoInt_cde FROM LigCde WHERE NoIntervention = '17489601'`);
    if (!lig.length) { console.log('Job 17489601 not found'); return; }
    const ligcdeId = lig[0].NOINT_LIGCDE;
    console.log('Job 17489601 -> ligcde', ligcdeId);
    ...
    const outPath = 'scripts/_scratch-snapshot-17489601-before.json';
