...
async function main() {
  ...
  try {
    // Trailing 365 days, matching what reman_analytics would scope to for a typical range.
    ...
    const rows = await conn.query(
      `SELECT l.NoInt_Ligcde FROM LigCde l
       ...
       WHERE l.NomClient = '' AND l."Soldée" = True AND l.Type_Service IN ('100','101','102','103')
         AND c.DateCommande BETWEEN '${fromYmd}' AND '${toYmdStr}'
       LIMIT 20000`
    );
    console.log(`Internal stock-processing closed jobs (bench Type_Service, trailing 365d intake): ${rows.length}`);

    const totalClosedBench = await conn.query(
      `SELECT l.NoInt_Ligcde FROM LigCde l
       LEFT JOIN Commande c ON l.NoInt_cde = c.NoInt_Cde
    ...
    console.log(`Total closed bench jobs (same window): ${totalClosedBench.length}`);
    console.log(`Fraction internal: ${(rows.length / totalClosedBench.length * 100).toFixed(1)}%`);
  } catch (e) {
