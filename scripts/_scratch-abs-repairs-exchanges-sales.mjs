...
async function main() {
  ...
  try {
    ...
    console.log(`Raw rows fetched: ${rows.length} (cap is 20000 - if at cap, older data may be missing)`);
    ...
    const byMonth = new Map(); // month -> {R, ES, V, total}
    let totals = { R: 0, ES: 0, V: 0 };
    for (const [, r] of latest) {
      if (!ALLOWED.has(r.TYPECODE)) continue;
      totals[r.TYPECODE]++;
      const ymd = toYmd(r.DATEDERNINTERV);
      if (!ymd) continue;
      const month = ymd.slice(0, 7);
      const cur = byMonth.get(month) || { R: 0, ES: 0, V: 0 };
      cur[r.TYPECODE]++;
      byMonth.set(month, cur);
    }
    console.log(`\nTotals: R=${totals.R} ES=${totals.ES} V=${totals.V} combined=${totals.R+totals.ES+totals.V}`);
    console.log('\nBy month (Repair / Exchange / Sold / Total):');
    for (const [m, c] of [...byMonth.entries()].sort()) {
      console.log(` ${m}: R=${c.R} ES=${c.ES} V=${c.V}  total=${c.R+c.ES+c.V}`);
    }
    const months = [...byMonth.entries()].filter(([m]) => m !== '2025-08' && !m.startsWith('2026-08'));
    const sum = months.reduce((s, [,c]) => s + c.R + c.ES + c.V, 0);
    console.log(`\nFull months only (excluding partial edges): ${months.length} months, total ${sum}, average ${(sum/months.length).toFixed(1)}/month`);
  } catch (e) {
