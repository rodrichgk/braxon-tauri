...
async function main() {
  ...
  try {
    const lig = await conn.query(`SELECT NoInt_Ligcde, NoInt_cde FROM LigCde WHERE NoIntervention = '17490801'`);
    if (!lig.length) { console.log('Job 17490801 not found'); return; }
    const ligcdeId = lig[0].NOINT_LIGCDE;
    console.log('Job 17490801 -> ligcde', ligcdeId);
    ...
    const outPath = 'scripts/_scratch-snapshot-17490801-before.json';
