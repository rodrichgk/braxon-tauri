...
async function main() {
  const before = JSON.parse(fs.readFileSync('scripts/_snapshot-17473101-before.json', 'utf8'));
  ...
  try {
    ...
    fs.writeFileSync('scripts/_snapshot-17473101-after.json', JSON.stringify({
      capturedAt: new Date().toISOString(), ligcde, interventions, detailInterv, maxInterventionIdAfterChange: maxInterv, commande,
    }, null, 2));
    console.log('\nAfter-snapshot saved to scripts/_snapshot-17473101-after.json');
  } catch (err) {
