...
async function main() {
  ...
  try {
    ...
    // Mirrors reman_analytics's new closed_sql exactly (with the NomClient fix).
    const fixed = await conn.query(
      `SELECT l.NoInt_Ligcde FROM LigCde l
       LEFT JOIN Commande c ON l.NoInt_cde = c.NoInt_Cde
       WHERE l."Soldée" = True AND c.DateCommande BETWEEN '${fromYmd}' AND '${toYmdStr}' AND l.NomClient <> ''
       LIMIT 20000`
    );
    console.log(`Fixed closed_sql (NomClient excluded): ${fixed.length}`);

    const unfixed = await conn.query(
      `SELECT l.NoInt_Ligcde FROM LigCde l
       LEFT JOIN Commande c ON l.NoInt_cde = c.NoInt_Cde
       WHERE l."Soldée" = True AND c.DateCommande BETWEEN '${fromYmd}' AND '${toYmdStr}'
       LIMIT 20000`
    );
    console.log(`Old closed_sql (no exclusion): ${unfixed.length}`);
    console.log(`Excluded: ${unfixed.length - fixed.length}`);
  } catch (e) {
