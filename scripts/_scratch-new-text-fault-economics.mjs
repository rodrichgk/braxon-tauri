...
async function checkFault(conn, label, famille, likeClause) {
  const rows = await conn.query(
    `SELECT l.NoInt_Ligcde, l.PrixHT FROM LigCde l LEFT JOIN ArticleMeteor am ON l.CodeArt = am.CodeArt
     WHERE am.Famille = '${famille}' AND (${likeClause}) LIMIT 3000`
  );
  ...
  console.log(`\n=== ${label} (${famille}) === ${rows.length} jobs`);
  console.log('Outcomes:', JSON.stringify(Object.fromEntries(outcomeCounts)));
  const rate = rows.length ? (repaired/rows.length*100).toFixed(1) : 0;
...
async function main() {
  ...
  try {
    await checkFault(conn, 'Wheels locking / brake problem', 'Bosch 8.0', "l.Observations LIKE '%ROUES%BLOQUEES%' OR l.Observations LIKE '%PROBLEME%FREINAGE%'");
    await checkFault(conn, 'Defaut Pompe', 'MK70', "l.Observations LIKE '%DEFAUT%POMPE%'");
    await checkFault(conn, 'Capteur Pression', 'MK60', "l.Observations LIKE '%CAPTEUR%PRESSION%'");
  } catch (e) {
