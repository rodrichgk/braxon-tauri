...
const filtered = [...byLig.values()].filter(r => {
  ...
  if (!r.DATELIMITELIVRAISON) return false; // require a date
...
// check the missing one specifically
const missRow = [...byLig.values()].find(r=>r.NOINTERVENTION==='17431601');
console.log('17431601 raw:', JSON.stringify(missRow));
await conn.close();
