...
async function main() {
  ...
  try {
    // Broad, all families: MOTEUR + PERMANENCE (catches "tourne en permanence",
    // "fonctionne en permanence", etc.) plus the narrower phrase checked earlier.
    const rows = await conn.query(
      `SELECT l.NoInt_Ligcde, l.CodeArt, am.Famille, l.Observations, l.PrixHT
    ...
    console.log(`Total matches (all families): ${rows.length}`);
    const famCounts = new Map();
    for (const r of rows) famCounts.set(r.FAMILLE, (famCounts.get(r.FAMILLE) || 0) + 1);
    console.log('By family:', JSON.stringify(Object.fromEntries(famCounts)));

    console.log('\nSample text:');
    for (const r of rows.slice(0, 20)) console.log(` - [${r.FAMILLE}] ${r.OBSERVATIONS}`);

    // Overlap with 5DF0/5DF1
    const withCode = rows.filter(r => /5DF0|5DF1/.test((r.OBSERVATIONS||'').toUpperCase()));
    console.log(`\nOf these, also mention 5DF0/5DF1: ${withCode.length}`);

    // Economics for the full set
    ...
    for (const r of rows) {
      ...
      const outcome = latest ? (latest.TYPELIBELLE || latest.TYPECODE) : '(no steps / open)';
