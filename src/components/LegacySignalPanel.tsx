...
export default function LegacySignalPanel({ sendMessage, isConnected }: LegacySignalPanelProps) {
  ...
  // Keep wssChannels fresh in the listener without re-subscribing
  ...
  useEffect(() => {
    const unsub = listen<string>('serial-data', e => {
      ...
      // AutoTest frequency feedback
      const freqMatch = line.match(/^Freq\s*:\s*([\d.]+)/);
      if (freqMatch) { setNanoFreq(parseFloat(freqMatch[1])); return; }
    ...
    <div className="card w-full">
      ...
      {hasWssChannels && (
        <div className="mt-4 pt-4 border-t border-border">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-text-primary">ABS Speed Readback</h3>
            {refKmhPerHz !== null && legacyFreq > 0 && (
              <span className="text-xs text-text-tertiary">
                Expected at {legacyFreq} Hz:{' '}
          ...
          <p className="text-[10px] text-text-tertiary mt-2 px-0.5">
            Speeds decoded from CAN bus using the fitted equations. Green = within 5% of expected.
            {!wssChannels.every(ch => ch !== null) && (
              <> Assign remaining wheels in CAN Analyzer.</>
            )}
          </p>
        </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-border flex items-start justify-between gap-4">
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-text-primary">Auto Test</h3>
          <p className="text-xs text-text-tertiary mt-0.5">
            Ramp up → ABS brake sim → coast (~10 s), Nano-generated
          </p>
          {isAutoTest && nanoFreq !== null && (
